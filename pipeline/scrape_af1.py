#!/usr/bin/env python3
"""Scrape AF1 data from the official Arena Football One DigitalShift site.

Outputs:
  - pipeline/raw/af1_players.json
  - pipeline/raw/af1_stats.json
  - pipeline/raw/af1_games.json
  - pipeline/raw/af1_raw.json

Notes:
  - Uses official theaf1.com / DigitalShift endpoints.
  - Parses per-game player rows from official team-stats tables.
...
PLAYERS_FILE = RAW_DIR / "af1_players.json"
STATS_FILE = RAW_DIR / "af1_stats.json"
GAMES_FILE = RAW_DIR / "af1_games.json"
RAW_FILE = RAW_DIR / "af1_raw.json"

API_URL = "https://web.api.digitalshift.ca"
STATS_URL = "https://www.theaf1.com/stats"
CLIENT_SERVICE_ID = "c68728ae-41f1-4e59-91f9-33e86f6f5d76"
LEAGUE_ID = 1999
SYNTHETIC_ID_START = 1_990_000
LEAGUE_NAME = "AF1"
...
def football_game_id_from_obj(game_obj: dict[str, Any]) -> str:
    ...
    return f"FOOTBALL_AF1_{year}_{month}_{day}_{away}@{home}"
...
def discover_season_ids() -> list[int]:
    r = requests.get(STATS_URL, headers=REQUEST_HEADERS, timeout=30)
    r.raise_for_status()
    body = r.text
...
def scrape_af1(season_ids: list[int], max_games: int | None = None) -> tuple[list[dict], list[dict], list[dict], dict]:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Scrape AF1 data from official DigitalShift endpoints")
    ap.add_argument(
        "--season-id",
        action="append",
        type=int,
        help="Season ID to scrape (repeatable). If omitted, discover from the official stats page.",
    )
    ...
    players, stats, games, raw_meta = scrape_af1(season_ids, max_games=args.max_games)
